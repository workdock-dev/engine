...
// NewHTTPServer initializes the application's HTTP server and registers all
// public endpoints.
//
...
func NewHTTPServer(address string, app application.App) (*Server, error) {
	s := &Server{
		address: address,
		app:     app,
	}
...
// domainErrToStatusCode translates application domain errors into the
// appropriate HTTP status codes.
//
// - Maps business-level errors to their corresponding HTTP responses.
// - Provides a consistent error contract for all HTTP handlers.
//
// Errors that are not explicitly recognized are treated as internal server
// errors.
func (s *Server) domainErrToStatusCode(err error) int {
