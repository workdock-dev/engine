...
func (q *EventQueue) Claim(ctx context.Context, owner string, nextAttemptAt time.Time) (*types.EventJob, error) {
	...
	if err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
			slog.Debug("[agent-session][event-queue][postgres] there aren't any job to claimns")
			return nil, nil
		}

		slog.Error("[agent-session][event-queue][postgres] failed to claim job", "owner", owner, "err", err)
		return nil, err
	}
...
func (q *EventQueue) Heartbeat(ctx context.Context, id string, leaseDuration time.Duration) error {
	...
	if err != nil {
		slog.Error("[agent-session][event-queue][postgres] failed to send jobs heartbeat", "event_identifier", id, "err", err)
		return err
	}

	if tags.RowsAffected() == 0 {
		slog.Error("[agent-session][event-queue][postgres] failed to set heartbeat, not affected rows", "event_identifier", id)
	}
...
//
//   - Transitions the job to the given succeeded-ish state.
...
func (q *EventQueue) Complete(ctx context.Context, id string, status types.EventJobStatus) error {
	tags, err := q.client.Exec(ctx, CompleteSql, id, status)

	if err != nil {
		slog.Error("[agent-session][event-queue][postgres] failed to send job as completed", "event_identifier", id, "err", err)
		return err
	}

	if tags.RowsAffected() == 0 {
		slog.Error("[agent-session][event-queue][postgres] failed to set job as completed, not affected rows", "event_identifier", id)
	}
...
func (q *EventQueue) Retry(ctx context.Context, id string, cause error, retryGracePeriod time.Duration) error {
	...
	if err != nil {
		slog.Error("[agent-session][event-queue][postgres] failed to send job for retry", "event_identifier", id, "err", err)
		return err
	}

	if tags.RowsAffected() == 0 {
		slog.Error("[agent-session][event-queue][postgres] failed to set job for retry, not affected rows", "event_identifier", id)
	}
...
func (q *EventQueue) Fail(ctx context.Context, id string, cause error) error {
	...
	if err != nil {
		slog.Error("[agent-session][event-queue][postgres] failed to send job for failure", "event_identifier", id, "err", err)
		return err
	}

	if tags.RowsAffected() == 0 {
		slog.Error("[agent-session][event-queue][postgres] failed to set job for failure, not affected rows", "event_identifier", id)
	}
