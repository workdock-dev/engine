...
func (s *ControllerSuite) TestOnAgentSessionArchive_DoneEvent_ArchivesAllSessionSandboxes() {
	...
	sessions = append(sessions, second)
	s.sessionRep.getAgentSessionsByIssueIdFn = func(ctx context.Context, issueId string) ([]*types.Session, error) {
	...
	s.Require().NoError(err)
	s.Require().Len(s.sandboxHdl.archived, 2)
	s.Equal("sess-1", s.sandboxHdl.archived[0].Session.Identifier)
...
func (s *ControllerSuite) TestOnAgentSessionArchive_SessionsLookupError() {
	...
	s.Error(err)
	s.ErrorContains(err, "sessions lookup failed")
	s.Empty(s.sandboxHdl.archived)
}
...
func (s *ControllerSuite) TestExecute_HarnessError_ContextCancelled_NoErrorMessage() {
	...
	s.harnessHdl.parseFn = func(
		...
		// Simulate a cancelled job: the scheduler handles the cancellation
		// separately, so no error message may be sent to the user.
		cancel()
...
func (s *ControllerSuite) TestExecute_TransitionsIssueToInReview_AfterContextCancellation() {
	...
	// used for the finalization calls.
	var transitionCtx, responseCtx context.Context
	s.agentHdl.transitionInReviewFn = func(ctx context.Context, issueId, accessToken string) error {
		transitionCtx = ctx
		return ctx.Err()
	}
	s.agentHdl.sendResponseFn = func(ctx context.Context, sessionId, accessToken, text string) error {
		responseCtx = ctx
		return ctx.Err()
	}
	...
	s.c.execute(ctx, &types.EventJob{SessionEventIdentifier: "evt-1"})
