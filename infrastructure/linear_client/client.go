...
func (s *LinearService) Webhook(ctx context.Context, req types.WebhookRequest) (any, types.WebhookEventType, error) {
	...
	if eventType == "Issue" {
		...
		if err := types.ValidateWebhookFreshness(time.UnixMilli(issuePayload.WebhookTimestamp), time.Now()); err != nil {
			slog.Error("webhook timestamp is outside the freshness window", "err", err)
			return nil, types.WebhookEventType_Unknown, types.ErrUnAuthorized
		}
	...
	if err := types.ValidateWebhookFreshness(time.UnixMilli(payload.WebhookTimestamp), time.Now()); err != nil {
		slog.Error("webhook timestamp is outside the freshness window", "err", err)
		return nil, types.WebhookEventType_Unknown, types.ErrUnAuthorized
	}
