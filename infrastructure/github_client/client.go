...
// New initializes the GitHub service with the credentials required
...
func New(config GitHubClientConfig) (*GitHubClient, error) {
...
// valid.
func (s *GitHubClient) CreateInstallationAccessToken(installationId int) (*github.InstallationAccessToken, error) {
	...
	if resp.StatusCode != http.StatusCreated {
		...
		if resp.StatusCode == http.StatusNotFound {
			return nil, fmt.Errorf("%w: unexpected status %d: %s", types.ErrGitHubInstallationUnavailable, resp.StatusCode, string(body))
		}

		return nil, fmt.Errorf("unexpected status %d: %s", resp.StatusCode, string(body))
	}

	var token github.InstallationAccessToken
...
func (s *GitHubClient) Webhook(ctx context.Context, req types.WebhookRequest) (*github.WebhookEvent, error) {
