...
func (t *WEventVerifier) Verify(_ context.Context, event *runners.WEvent) (*runners.VerifiedWEvent, error) {
	if !t.isAllowedIP(event) {
		...
		return nil, runners.ErrWForBidden
	}
...
// processing.
func (t *WEventVerifier) isAllowedIP(event *runners.WEvent) bool {
	ip := t.clientIP(event)
...
func (t *WEventVerifier) clientIP(event *runners.WEvent) string {
...
// same payload twice.
func (c *WEventConsumer) Consume(_ context.Context, event *runners.VerifiedWEvent) error {
	if event.WEventType == WEventType_Issue {
		var payload IssueStatusChangePayload
	...
	if event.WEventType == WEventType_AgentSession {
		var payload AgentSessionEventData
...
func (c *WEventConsumer) verifyTimestampRecency(timestamp int64) error {
	...
	if diff < -60*time.Second || diff > 60*time.Second {
		...
		return runners.ErrWUnAuthorized
	}
