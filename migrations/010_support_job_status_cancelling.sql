...
-- Everything that uses the 'cancelling' job status added by the previous
-- migration. Because the sandbox is shared across the session's jobs, a job
-- queued while a sibling is 'cancelling' must not be claimed until the
