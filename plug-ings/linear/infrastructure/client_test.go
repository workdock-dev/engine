...
func (s *LinearServiceSuite) TestWebhook_IPDenied() {
	...
	s.ErrorIs(err, shared.ErrForbidden)
}
...
func (s *LinearServiceSuite) TestWebhook_Success() {
	...
	s.NoError(err)
	s.Equal(shared.WebhookEventType_AIRequest, eventType)
	parsed, ok := result.(*types.AgentSessionEventData)
	s.Require().True(ok)
...
func (s *LinearServiceSuite) TestIsAllowedIP_Allowed() {
	svc := s.newServiceNoServer("", "")
	req := shared.WebhookRequest{RemoteAddr: "10.0.0.1:1234", Headers: map[string][]string{}}
	s.True(svc.isAllowedIP(req))
}

func (s *LinearServiceSuite) TestIsAllowedIP_Denied() {
	svc := s.newServiceNoServer("", "")
	req := shared.WebhookRequest{RemoteAddr: "10.99.99.99:1234", Headers: map[string][]string{}}
	s.False(svc.isAllowedIP(req))
}

func (s *LinearServiceSuite) TestIsAllowedIP_EmptyList() {
	svc, err := NewClient(types.Config{IPs: []string{}}, nil)
	s.Require().NoError(err)
	req := shared.WebhookRequest{RemoteAddr: "10.0.0.1:1234", Headers: map[string][]string{}}
	s.False(svc.isAllowedIP(req))
}
